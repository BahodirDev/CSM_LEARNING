...
function Register() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            let { data } = await axios.post(`/register`, { name, login, password });
            console.log(data);
            if (data?.error) {
                ...
                <div className="row">
                    <div className="col-md-6 offset-md-3 mt-5">
                        <form onSubmit={handleSubmit}>
                            <input type="text" value={name} className="form-control my-2 p-2" onChange={(e) => setName(e.target.value)} placeholder="ISMINGIZNI KIRITING" />
                            ...
                            {/* <input type="text" value={address} className="form-control my-2 p-2" onChange={(e) => setAddress(e.target.value)} placeholder="Iltimos, manzilingizni kiriting" /> */}
